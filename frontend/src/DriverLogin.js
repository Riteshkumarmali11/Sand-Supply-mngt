...
const DriverLogin = () => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            const response = await axios.post('http://localhost:5000/driverLogin', { mobile, password });
            const { token, username } = response.data;
            toast.success("Login Successfully");
            console.log("response", response);
            localStorage.setItem('token', token);
            localStorage.setItem('username', username); 
            navigate('/driverNavbar')
        } catch (error) {
            console.log("error");
                ...
                {loading ? (
                    ...
                    <div className="h-screen flex justify-center items-center">
                        <div className="bg-white mx-4 p-8 rounded-4 shadow-md w-full md:w-1/2 lg:w-1/3">
                            <h1 className="text-3xl font-bold mb-8 text-center">Driver Login</h1>
                            <div>
