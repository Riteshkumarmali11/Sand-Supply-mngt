...
const PurchaseComponent = () => {
    ...
    const fetchInventoryData = async () => {
        try {
            ...
            if (success) {
                ...
                console.log("Purchase Inventory", data);
            } else {
    ...
    const handlePdfView = (ownerName, month) => {
        const ownerMonthTransactions = inventoryData.filter(transaction => {
            ...
            return transaction.selectSiteName === ownerName && transactionMonth === month;
        });
    ...
    const handleFilter = (fromDate, toDate) => {
        ...
        if (ownerNameFilter) {
            filteredTransactions = filteredTransactions.filter(transaction => transaction.selectSiteName === ownerNameFilter);
        }
    ...
    const processData = (data) => {
        ...
        data.forEach(transaction => {
            const month = transaction.paymentdate.split('-')[1];
            const key = `${transaction.selectSiteName}-${month}`;
            if (!processedData[key]) {
                processedData[key] = {
                    ...transaction,
                    month,
                    totalCredit: parseFloat(transaction.payableAmount.replace(/,/g, ''))
                };
            } else {
                processedData[key].totalCredit += parseFloat(transaction.payableAmount.replace(/,/g, ''));
            }
        });
    ...
    const calculateClosingBalance = (transactions) => {
        ...
        const totalCredits = transactions.reduce((total, transaction) => {
            return total + parseFloat(transaction.payableAmount.replace(/,/g, '') || 0);
        }, 0);
                            ...
                            <tbody>
                                {filteredInventoryData.map((transaction, index) => (
                                    <tr key={index}>
                                        ...
                                        <td>{transaction.selectSiteName}</td>
                                        <td>{formatCreditValue(transaction.totalCredit)}</td>
                                        <td>0</td>
                                        <td>
                                            <FaFilePdf style={{ color: 'orange' }}
                                                className="pdf-icon"
                                                onClick={() => handlePdfView(transaction.selectSiteName, new Date(transaction.paymentdate).getMonth() + 1)}
                                            />
                                        </td>
                                        <td style={{ textAlign: 'center' }}>
                                            <Link to={`/purchaseproduct/${transaction.id}?date=${transaction.paymentdate}`}>
                        ...
                        <ModalLayout>
                            <div style={{ fontWeight: 'bolder', textAlign: 'right' }} className='mb-10'>
                                {selectedOwnerTransactions.map((transaction, index) => (
                                    <h1>Opening Balance :{transaction.openingbalance}</h1>
                                ))}
                            </div>
                            <table className='s-table'>
                                <thead className='bg-orange-100'>
                                    <tr>
                                        ...
                                        <th className='bg-dark-200'>Site Name</th>
