...
const PurchaseInventory = () => {
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    try {
      const formattedOwnerName = `${selectedOwnerName}`;
      const response = await axios.post(`http://localhost:5000/PurchaseInventoryRegister`, {
        ...formData,
  ...
  const handleChange = (e) => {
    ...
    if (name === 'selectedOwnerName') {
      ...
      const selectedRegistration = registrations.find(registration => registration.companyname === value);
  ...
  const calculateTotal = () => {
    ...
    registrations.forEach(registration => {
      if (registration.companyname === selectedOwnerName) {
  ...
  const filteredRegistrations = registrations.filter(registration => registration.companyname === selectedOwnerName);
    ...
    <>
      ...
      <Layout>
        <div className="registration-form-container">
          <ToastContainer />
          <h1 style={{ textAlign: 'center', fontSize: '30px', padding: '20px' }}><strong>Purchase Ledger</strong></h1>
          <form encType="multipart/form-data">
            <div className='form-row'>
              ...
              <div className="form-group">
                <label>Payable Payment Mode</label>
              ...
              <div className='form-group'>
                <label>Payable Payment Date</label>
